...
class utakmicaModel {
	...
	public function save($post, $update = false) {
	  // @todo: Validate post
	  ...
	  //update:
	    ...
	    $sifra_utakmice = $this->sifra_utakmice;
	    $stmt = $this->register->db_conn->prepare('UPDATE utakmice
				SET godina_sezone = ?, datum = ?, sifra_lige = ?, domacin = ?, gost = ?, domacin_ft_golova = ? ,
	          gost_ft_golova = ?, trazi_se_pregled = ?, pregledana = ?
	      WHERE sifra_utakmice = ?');
	    $stmt->bind_param('issssiiisi', $this->godina_sezone, $this->datum, $this->sifra_lige, $this->domacin, $this->gost,
	        $this->domacin_golova, $this->gost_golova, $this->pregled, $this->pregledana, $sifra_utakmice);
	    if (!$stmt->execute()) {
	      $this->register->infos->set_error($stmt->error);
	    }
	    else {
		...
		if (isset($sifra_utakmice)) {
		  // sacuvaj pozicije sudija na utakmici:
		  include_once 'model/sudije.model.php';
	    sudijaModel::save_sudije($sifra_utakmice, $sudija, $prekrsaji, $this->register);
		}
